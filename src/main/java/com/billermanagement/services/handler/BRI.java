...
@Service
@Qualifier("bri")
public class BRI extends BillerRequest {
    @Autowired
    ApiConfig apiConfig;
    ...
    public Object process(RequestVO vo, BillerResult billerResult) throws Exception {
        try {
            ...
            System.out.println(">>> BRI.category:" + briid);
            ...
            if (initDB.get("BRI.Custodian") != null){
                sourceAccount = initDB.get("BRI.Custodian");
                paramsVOList.add(createParamsVO("sourceAccount",sourceAccount));
            }

            if (initDB.get("BRI.Transfer.FeeType") != null){
                paramsVOList.add(createParamsVO("FeeType",initDB.get("BRI.Transfer.FeeType")));
            }

            SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
            paramsVOList.add(createParamsVO("TransDateTime",sdf.format(new Date())));

            if (initDB.get("BRI.Transfer.Remark") != null){
                paramsVOList.add(createParamsVO("Remark",initDB.get("BRI.Transfer.Remark")));
            }
            vo.setParams(paramsVOList);
            String[] request = getRequest(vo, billerResult, 20);
            Map<String,String> mapHeader = getHttpHeader(request[1], request[0], briid);
            logger.info("BRI." + briid + ".request: " + request[0]);
            String msgResponse = sendRequest(request[1], mapHeader, request[0]);
            logger.info("BRI." + briid + ".response: " + msgResponse);

            vo.setAccountCode(acctCode);
            Object obj = getResponse(vo, billerResult, msgResponse);

            if (initDB.get("BRI.async.code") != null) {
                asyncStatus = initDB.get("BRI.async.code").split(";");
            }
...
//    }
    ...
    private Map<String,String> getHttpHeader(String url, String data, String briid) throws Exception {
        String clientSecretKey = new StringBuilder("bri.client.secret.").append(briid).toString();
        String token = getToken(briid);
    ...
    private String getToken(String briid) throws Exception {
        ...
        if (tokenExpired == null) {
            ...
            if (elapsed >= Integer.parseInt(tokenExpired)) requestToken(briid);
        }

        return briMap.get(tokenMapKey);
    }

    private String requestToken(String briid) throws Exception {
        //String url = "https://partner.api.bri.co.id/oauth/client_credential/accesstoken?grant_type=client_credentials";
        String clientIdKey = "bri.client.id." + briid;
        String clientSecretKey = "bri.client.secret." + briid;
        ...
        System.out.println(">>> BRI.briid:" + briid);
        System.out.println(">>> BRI.url:" + InitDB.getInstance().get("bri.token.url"));
        System.out.println(">>> BRI.param.url:" + urlParameters);

        String httpRes = sendRequest(InitDB.getInstance().get("bri.token.url"), map, urlParameters);
        ...
        briMap.put("Token." + briid, token);
        briMap.put("Token_Start." + briid, Long.toString(System.currentTimeMillis()));
        briMap.put("Token_Expired." + briid, Integer.toString(tokenExpired));
    ...
    public void checkStatus(String transId, int handler) throws Exception {
        System.out.println(">>> BRI.checkStatus.start");
...
//            vo.setNoReferral(requestVO.getReffId());
            ...
            if (success) {
                ...
                if (record.getSlaTime() == null || record.getSlaTime().toString().isEmpty()){
                    ...
                    int slaInSeconds=0;
                    if (initDB.get("BRI.Pending.SLA") !=null){
                        slaInSeconds = Integer.valueOf(initDB.get("BRI.Pending.SLA"));
                    }
            ...
            System.out.println(">>> BRI.checkStatus.end");
        }
    }

    @Transactional
    private boolean checkTrans(String message, TransTmp record) throws Exception {
        System.out.println(">>> BRI.checkTrans.start");
        String msgResponse;
        TransHistory transHistory = new TransHistory();
        try {
            Map<String,String> mapHeader = getHttpHeader(InitDB.getInstance().get("bri.status.url"), message, "transfer");
            logger.info("BRI.checkStatus.request: " + message);
            msgResponse = sendRequest(InitDB.getInstance().get("bri.status.url"), mapHeader, message);
            logger.info("BRI.checkStatus.response: " + msgResponse);
        } catch (Exception e) {
            return false;
        }
        ...
        if (isPending)  {
            ...
            try {
                ...
                logger.info("BRI.checkTrans.error : " + ex.getMessage());
                return false;
            }
            ...
            System.out.println(">>> BRI.checkTrans.end");
            return true;
        }
    }
